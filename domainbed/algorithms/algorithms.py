...
class ERM_NU(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ERM_NU, self).__init__(input_shape, num_classes, num_domains, hparams)
    ...
    def update(self, x, y, step, **kwargs):
        ...
        preds = self.classifier(all_feature)
        loss = F.cross_entropy(preds, all_y)
        
        if self.theta > 0 and step > 100:
            u,s,v = torch.svd(all_feature, compute_uv=True)
            loss = loss + self.theta * torch.sum(torch.abs(s)) / all_feature.shape[0]
